  ...
  <div v-if="state.config" class="w-full">
    <header class="text-center mb-8">
      <div v-if="state.config.imageUrl" class="mb-6">
        <img
          :src="state.config.imageUrl"
          alt="Turnierbild"
          class="w-full h-48 md:h-64 object-cover rounded-xl shadow-lg"
        />
      </div>
      <h1
        id="display-tournament-name"
        class="text-5xl font-extrabold tracking-tight text-gray-900 dark:text-white"
      >
        {{ state.config.tournamentName }}
      </h1>
      <div
        v-if="state.config.location && state.config.location.name"
        id="display-location"
        class="mt-2 text-md text-gray-500 dark:text-gray-400"
      >
      ...
      ></div>
      <div
        class="mt-4 text-sm text-gray-500 dark:text-gray-400 flex flex-wrap items-center justify-center gap-x-4 gap-y-2"
      >
        <span
          >Turnier-ID:
          <strong class="select-all">{{ state.config.id }}</strong></span
        >
        <div class="flex gap-2">
          <button
            class="px-2 py-1 text-xs bg-gray-200 dark:bg-gray-700 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600"
            @click="copyLink('host')"
          >
            Admin-Link kopieren
          </button>
          <button
            ...
            @click="copyLink('readonly')"
          >
            Zuschauer-Link kopieren
          </button>
        </div>
      </div>
      <div
        id="round-navigation"
        class="flex items-center justify-center mt-2 text-2xl text-gray-600 dark:text-gray-300"
      >
        <button
          class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-25 disabled:cursor-not-allowed"
          :disabled="state.viewingRound <= 1"
          @click="navigateRounds('prev')"
        >
          <svg
            class="w-6 h-6"
            fill="none"
        ...
        <h2 class="mx-4">
          Runde <span class="font-bold">{{ state.viewingRound }}</span>
        </h2>
        <button
          class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-25 disabled:cursor-not-allowed"
          :disabled="state.viewingRound >= state.currentRound"
          @click="navigateRounds('next')"
        >
          <svg
            class="w-6 h-6"
            fill="none"
    ...
    <div
      v-if="isViewingPast"
      class="text-center bg-yellow-100 dark:bg-yellow-900 border-l-4 border-yellow-500 text-yellow-700 dark:text-yellow-200 p-4 mb-6 rounded-md"
    >
      <p>
        Sie betrachten eine vergangene Runde. Die Ergebnisse sind
        schreibgeschützt.
      </p>
    </div>
    ...
    <div id="view-mode-toggle" class="flex justify-center mb-6">
      <fieldset class="inline-flex rounded-md shadow-sm">
        ...
        <button
          ...
          :class="{ 'view-mode-btn-active': state.matchViewMode === 'group' }"
          @click="setMatchViewMode('group')"
        >
          Nach Gruppen
        </button>
        <button
          type="button"
          class="view-mode-btn px-4 py-2 text-sm font-medium"
          :class="{ 'view-mode-btn-active': state.matchViewMode === 'chrono' }"
          @click="setMatchViewMode('chrono')"
        >
          Chronologisch
        </button>
      </fieldset>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
      <StandingsTable
        v-for="(teamIds, name) in state.groups"
        :key="name"
        :group-name="name"
        :team-ids="teamIds"
      />
...
const isActionButtonDisabled = computed(() => {
  ...
  const currentRoundData = state.rounds.find(
...
const actionButtonClass = computed(() => {
  const base =
    'text-white font-bold py-3 px-8 rounded-lg shadow-md transition-transform transform hover:scale-105';
  if (isViewingPast.value) return `${base} bg-blue-600 hover:bg-blue-700`;
  if (isActionButtonDisabled.value)
