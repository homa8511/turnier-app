<template>
    <div v-if="state.config" class="w-full">
        <header class="text-center mb-8">
            ...
            <div v-if="state.config.description" class="prose prose-sm dark:prose-invert max-w-none mx-auto mt-4 text-left text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 p-4 rounded-lg" v-html="state.config.description"></div>
            <div class="mt-4 text-sm text-gray-500 dark:text-gray-400 flex flex-wrap items-center justify-center gap-x-4 gap-y-2">
                ...
                <div class="flex gap-2">
                    <button @click="copyLink('host')" class="px-2 py-1 text-xs bg-gray-200 dark:bg-gray-700 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600">Admin-Link kopieren</button>
                    <button @click="copyLink('readonly')" class="px-2 py-1 text-xs bg-blue-500 text-white rounded-md hover:bg-blue-600">Zuschauer-Link kopieren</button>
                </div>
            </div>
            <div id="round-navigation" class="flex items-center justify-center mt-2 text-2xl text-gray-600 dark:text-gray-300">
                <button @click="navigateRounds('prev')" :disabled="state.viewingRound <= 1" class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-25 disabled:cursor-not-allowed">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                </button>
                <h2 class="mx-4">Runde <span class="font-bold">{{ state.viewingRound }}</span></h2>
                <button @click="navigateRounds('next')" :disabled="state.viewingRound >= state.currentRound" class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-25 disabled:cursor-not-allowed">
                    <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
                </button>
            </div>
        </header>

        <div v-if="state.viewingRound < state.currentRound" class="text-center bg-yellow-100 dark:bg-yellow-900 border-l-4 border-yellow-500 text-yellow-700 dark:text-yellow-200 p-4 mb-6 rounded-md">
        ...
        <div id="view-mode-toggle" class="flex justify-center mb-6">
            <div class="inline-flex rounded-md shadow-sm" role="group">
                <button type="button" @click="setMatchViewMode('group')" class="view-mode-btn px-4 py-2 text-sm font-medium" :class="{'view-mode-btn-active': state.matchViewMode === 'group'}">
                    Nach Gruppen
                </button>
                <button type="button" @click="setMatchViewMode('chrono')" class="view-mode-btn px-4 py-2 text-sm font-medium" :class="{'view-mode-btn-active': state.matchViewMode === 'chrono'}">
                    Chronologisch
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
            <StandingsTable v-for="(group, name) in state.groups" :key="name" :group-name="name" :team-ids="group" />
        </div>
        
        <div v-if="!state.isReadOnly" id="action-button-container" class="mt-8 text-center">
            <button id="action-button" @click="handleActionButtonClick" :disabled="isActionButtonDisabled" :class="actionButtonClass">
                {{ actionButtonText }}
            </button>
        </div>
    </div>
