...
export async function createTournament(
  config: Omit<TournamentConfig, 'id'>
): Promise<Tournament> {
...
export async function startTournament(
  id: string,
  teams: { name: string; group: string; logo: string }[]
): Promise<Tournament> {
...
export async function updateTournamentConfig(
  id: string,
  config: Partial<Omit<TournamentConfig, 'id'>>
): Promise<Tournament> {
...
export async function loadTournament(id: string): Promise<Tournament | null> {
  if (!id) {
    void Swal.fire('Fehler', 'Bitte geben Sie eine Turnier-ID ein.', 'warning');
    return null;
  }
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        void Swal.fire(
          'Fehler',
          `Kein Turnier mit der ID "${id}" gefunden.`,
          'error'
        );
      } else {
